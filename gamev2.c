...
int main(void)
{
    const int screenWidth = 1000; 
    const int screenHeight = 1000;
    InitWindow(screenWidth, screenHeight, "FALL BALL v1.9");
    int playerSize = 50; // Initial size of the player
    int radii1 = 20;
    int score = 0;
    int ballx = 500;
    ...
    while (!WindowShouldClose())
    {
        ...
        if (score >= 30){
            ClearBackground(RAYWHITE);
            DrawText("WOW INCREDIBLE",400,450,50,RED);
        }

        if (score >= 100){
            ClearBackground(RAYWHITE);
            DrawText("WELL YOU SURVIVED FOR LONG",400,450,50,RED);
            DrawText("IM pretty sure this text is broken so just die",400,450,50,RED);
